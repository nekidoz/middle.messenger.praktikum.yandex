export default `<nav class=
{{#if class }}
    "{{class}}"
{{else}}
    "menu-page-menu"
{{/if}}
 >
    {{> Link href="#" class="menu-page-menu-item" data-page="login" text="Вход"}}
    {{> Link href="#" class="menu-page-menu-item" data-page="signup" text="Регистрация"}}
    {{> Link href="#" class="menu-page-menu-item" data-page="profile" text="Профиль"}}
    {{> Link href="#" class="menu-page-menu-item" data-page="chats" text="Чаты"}}
    {{> Link href="#" class="menu-page-menu-item" data-page="page404" text="404"}}
    {{> Link href="#" class="menu-page-menu-item" data-page="page5xx" text="5xx"}}
</nav>`
